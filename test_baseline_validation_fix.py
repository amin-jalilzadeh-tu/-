#!/usr/bin/env python3
"""
Test the baseline validation with the fixed date column detection
"""
...
def test_baseline_validation():
    """Test baseline validation with fixed date detection"""
    ...
    # Configuration
    config = {
        "target_frequency": "monthly",  # Use monthly to test the fix
        "thresholds": {
            "cvrmse": 25.0,
            "nmbe": 8.0
        }
    ...
    print("Running baseline validation with fixed date detection...")
    
    # Run validation without variants (baseline only)
    ...
    if parquet_file.exists():
        ...
        print(f"  - Variables validated: {df['real_variable'].unique() if 'real_variable' in df.columns else 'N/A'}")
    else:
        print(f"\n✗ FAILED: No parquet file found at {parquet_file}")
        
        # Check if any results were generated
        if results and 'validation_results' in results:
            print(f"\nValidation results were generated but not saved to parquet:")
            print(f"  - Number of results: {len(results['validation_results'])}")
    
    return results

if __name__ == "__main__":
    results = test_baseline_validation()