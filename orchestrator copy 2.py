    ...
    if perform_modification:
        with step_timer(logger, "IDF modification"):
            ...
            if output_options.get("save_report", True):
                ...
                # Simple JSON report (always generate)
                report_data = {
                    'metadata': {
                        'job_id': job_id,
                        'timestamp': datetime.now().isoformat(),
                ...
                report_path = modified_idfs_dir / f"modification_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
            ...
            # -------------------------------------------------------------------------
            # 13a) Run simulations on modified IDFs (if enabled)
            # -------------------------------------------------------------------------
            post_mod_cfg = modification_cfg.get("post_modification", {})
            if post_mod_cfg.get("run_simulations", False) and modified_building_data:
                    ...
                    if os.path.isfile(idf_map_csv):
                        ...
                        if modified_rows:
                            ...
                            try:
                                ...
                                # -------------------------------------------------------------------------
                                # 13b) Parse modified simulation results (if enabled)
                                # -------------------------------------------------------------------------
                                ...
                                if parse_cfg:
                                    with step_timer(logger, "parsing modified results"):
                                        ...
                                        pairs_with_mapping = prepare_idf_sql_pairs_with_mapping(
                                            ...
                                            idf_dir=str(modified_idfs_dir),
                                            idf_map_csv=idf_map_csv  # Use original mapping
                                        )
                                        ...
                                        if pairs_with_mapping:
                                            ...
                                            analyzer.analyze_multiple_files(
                                                pairs_with_mapping,
                                                parse_idf=parse_cfg.get("parse_idf", False),
                                                parse_sql=parse_cfg.get("parse_sql", True),
                                            ...
                                            parsing_summary = {
                                                ...
                                                'files_parsed': len(pairs_with_mapping),
                                                'parser_output_dir': parser_output_dir,
                                                'timestamp': datetime.now().isoformat(),
                                                'parsed_data_info': parsed_info
                                            }
                                            ...
                                            logger.warning("[WARN] No modified simulation results found to parse")
                ...
                if not post_mod_cfg.get("run_simulations", False):
                    logger.info("[INFO] Post-modification simulations disabled in config")
                else:
                    logger.info("[INFO] No modified buildings to simulate")
