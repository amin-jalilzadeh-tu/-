...
def find_dhw_overrides(
    building_id,
    dhw_key,
    user_config,
    building_function=None,  # <--- NEW
    age_range=None           # <--- NEW
):
    """
    ...
    for row in (user_config or []):
        # 1) If row has "building_id", must match building_id
        if "building_id" in row:
            if row["building_id"] is not None and row["building_id"] != building_id:
                continue

        # 2) If row has "dhw_key", must match dhw_key
        if "dhw_key" in row:
            if row["dhw_key"] is not None and row["dhw_key"] != dhw_key:
                continue

        # 3) If row has "building_function", must match (case-insensitive)
        if "building_function" in row:
            if row["building_function"] is not None:
                if building_function is None:
                    continue
                if row["building_function"].lower() != building_function.lower():
                    continue

        # 4) If row has "age_range", must match exactly
        if "age_range" in row:
        ...
        # If we haven't continued away, it's a match
        matches.append(row)
    return matches
...
def assign_dhw_parameters(
    ...
    use_nta: bool = False,
    building_function: str = None,  # <--- NEW
    age_range: str = None           # <--- NEW
):
    """
    ...
    # 1) Decide calibration stage => fallback to "pre_calibration" if missing
    ...
    # 2) Gather user config overrides, including building_function + age_range
    matches = find_dhw_overrides(
        building_id=building_id,
        dhw_key=dhw_key,
        user_config=user_config_dhw,
        building_function=building_function,  # <--- pass these
        age_range=age_range                  # <--- pass these
    )
    ...
    # 2b) Apply user_config overrides from each matching row
    for row in matches:
    ...
    if building_row is not None:
        bldg_func_l = str(building_row.get("building_function", "")).lower()
        if "residential" in bldg_func_l:
