...
def create_schedule_from_pattern(
    ...
    Returns:
        The schedule object or None if creation fails.
    """
    limits_obj = ensure_schedule_type_limits(idf, schedule_type_limits) # Ensure limits exist
    if not limits_obj:
    ...
    try:
        ...
        # Sort pattern by end_hour just in case
        sorted_pattern = sorted(pattern, key=lambda x: x[1])

        # Add fields for each time step in the pattern
        last_end_hour = 0
        for (start_hr, end_hr, val) in sorted_pattern:
            ...
            if not (isinstance(start_hr, int) and isinstance(end_hr, int) and isinstance(val, (int, float))):
                 ...
                 continue
            if end_hr <= last_end_hour or end_hr > 24 or start_hr < 0 or start_hr >= end_hr:
                 print(f"[WARNING] Invalid time segment ({start_hr}-{end_hr}) in pattern for {sched_name}. Skipping segment.")
                 continue
            # Format "Until: HH:MM,Value"
            line_str = f"Until: {end_hr:02d}:00,{val:.4f}" # Format value for consistency
            # Dynamically assign fields checking bounds if necessary (Eppy/Geomeppy usually handle this)
            sched_obj[f"Field_{field_idx}"] = line_str
            field_idx += 1
            last_end_hour = end_hr

        # Check if the pattern covers the full 24 hours
        if last_end_hour < 24:
             print(f"[WARNING] Pattern for schedule {sched_name} does not cover until 24:00. EnergyPlus might fill with last value.")
...
def create_schedule_from_weekday_weekend_pattern(
    ...
    weekend_pattern: SchedulePattern,
    ...
    Returns:
        The schedule object or None if creation fails.
    """
    limits_obj = ensure_schedule_type_limits(idf, schedule_type_limits)
    if not limits_obj:
         print(f"[ERROR] Could not ensure ScheduleTypeLimits '{schedule_type_limits}' for {sched_name}.")
         return None
    ...
    if not weekday_pattern or not weekend_pattern:
    ...
    try:
        ...
        if last_end_hour_wd < 24: print(f"[WARNING] Weekday pattern for {sched_name} doesn't cover 24h.")
        ...
        if last_end_hour_we < 24: print(f"[WARNING] Weekend pattern for {sched_name} doesn't cover 24h.")

        return sched_obj
    except Exception as e:
        print(f"[ERROR] Failed to create SCHEDULE:COMPACT {sched_name} from patterns: {e}")
        return None
...
def ensure_dynamic_schedule(
    ...
    weekend_pattern: Optional[SchedulePattern] = None,
    schedule_type_limits="Fraction"
) -> Optional[Any]:
    """
    Convenience function to create/retrieve a schedule:
      - If only weekday_pattern is provided, applies it to AllDays.
      - If both are provided, creates a Weekday/Weekend schedule.
      - If neither pattern is provided, falls back to creating an AlwaysOn schedule.

    Args:
        idf: geomeppy IDF instance.
        sched_name: Desired name for the schedule.
        weekday_pattern: Optional pattern for weekdays.
        weekend_pattern: Optional pattern for weekends.
        schedule_type_limits: Name of the limits object.

    Returns:
        The created or retrieved schedule object, or None on failure.
    """
    # Check if already exists first (case-insensitive)
    ...
    if existing_constant:
        # Found a Constant schedule. If no patterns were provided, this is the fallback.
        # If patterns *were* provided, maybe warn that a Constant exists?
        if not weekday_pattern and not weekend_pattern:
            return existing_constant
        else:
             print(f"[WARNING] Dynamic patterns provided for '{sched_name}', but a SCHEDULE:CONSTANT with this name already exists. Returning the Constant.")
             return existing_constant

    # Create new based on provided patterns
    ...
    elif weekday_pattern: # Only weekday pattern provided
        return create_schedule_from_pattern(
            idf, sched_name, weekday_pattern, schedule_type_limits
        )
    else: # No patterns provided, fallback to AlwaysOn
        print(f"[INFO] No patterns provided for '{sched_name}'. Falling back to AlwaysOn schedule.")
