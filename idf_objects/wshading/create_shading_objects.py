...
def add_shading_objects(
    ...
    create_blinds : bool
        ...
        + WindowProperty:ShadingControl) for fenestration surfaces.
    ...
    if create_blinds:
        try:
            ...
            if not fen_surfaces:
                logger.info("No 'FENESTRATIONSURFACE:DETAILED' objects found in IDF. Skipping blind creation.")
        except Exception as e:
            logger.error(f"Could not retrieve FENESTRATIONSURFACE:DETAILED from IDF: {e}")
            fen_surfaces = []
        ...
        for fen_idx, fen in enumerate(fen_surfaces):
            try:
                ...
                logger.info(f"Processing blinds for window: {window_id} using shading_type_key: '{shading_type_key}'")
                ...
                if not shading_params:
                    logger.warning(f"No shading parameters resolved for window '{window_id}' with key '{shading_type_key}'. Skipping blind creation for this window.")
                    continue

                # 2) Create a WindowMaterial:Blind object
                blind_mat_name = shading_params.get("blind_name", "DefaultBlindMaterial") + f"_{window_id}"
                
                # Check if blind material already exists to prevent duplicates
                existing_blind_mats = [bm.Name for bm in idf.idfobjects.get("WINDOWMATERIAL:BLIND", [])]
                if blind_mat_name in existing_blind_mats:
                    logger.info(f"WindowMaterial:Blind '{blind_mat_name}' already exists. Reusing.")
                    ...
                    blind_mat.Slat_Width = shading_params.get("slat_width", 0.025) # Default 25mm
                    blind_mat.Slat_Separation = shading_params.get("slat_separation", 0.020) # Default 20mm
                    blind_mat.Slat_Thickness = shading_params.get("slat_thickness", 0.001) # Default 1mm
                    blind_mat.Slat_Angle = shading_params.get("slat_angle_deg", 45.0) # Default 45 degrees
                    blind_mat.Slat_Conductivity = shading_params.get("slat_conductivity", 160.0) # Default for Aluminum
                    ...
                    blind_mat.Back_Side_Slat_Beam_Solar_Reflectance = sbsr # Assuming symmetric
                    ...
                    blind_mat.Back_Side_Slat_Diffuse_Solar_Reflectance = sdsr # Assuming symmetric
                    ...
                    blind_mat.Back_Side_Slat_Beam_Visible_Reflectance = sbvr # Assuming symmetric
                    ...
                    blind_mat.Back_Side_Slat_Diffuse_Visible_Reflectance = sdvr # Assuming symmetric
                    ...
                    blind_mat.Back_Side_Slat_Infrared_Hemispherical_Emissivity = sir_em # Assuming symmetric
                    ...
                    if "blind_to_glass_distance" in shading_params:
                         blind_mat.Distance_between_Slat_and_Glazing = shading_params.get("blind_to_glass_distance")
                    if "blind_top_opening_multiplier" in shading_params:
                         blind_mat.Slat_Opening_Multiplier = shading_params.get("blind_top_opening_multiplier") # Note: E+ has one Slat_Opening_Multiplier
                ...
                # 3) Create WindowProperty:ShadingControl object
                shading_ctrl_name = f"ShadingCtrl_{window_id}"
                
                # Check if shading control already exists
                existing_ctrls = [sc.Name for sc in idf.idfobjects.get("WINDOWPROPERTY:SHADINGCONTROL", [])]
                if shading_ctrl_name in existing_ctrls:
                    logger.info(f"WindowProperty:ShadingControl '{shading_ctrl_name}' already exists. Reusing.")
                    ...
                    shading_ctrl.Shading_Type = "ExteriorBlind" # Or "InteriorBlind", "ExteriorScreen" etc.
                                                                # This should ideally come from shading_params or shading_type_key logic
                    shading_ctrl.Shading_Device_Material_Name = blind_mat_name # Use the potentially unique name
                    
                    # Defaulting to FixedSlatAngle. More complex control types would need schedule names etc.
                    shading_ctrl.Type_of_Slats_Control_for_Blinds = "FixedSlatAngle" # E.g., FixedSlatAngle, ScheduledSlatAngle
                    ...
                    shading_ctrl.Shading_Control_Is_Scheduled = "No" # Set to "Yes" if using a schedule
                    # Shading_Control_Schedule_Name would be needed if "Yes"

                    shading_ctrl.Glare_Control_Is_Active = "No" # Defaulting, can be "Yes"
                    # Other fields like Setpoint, ShadingControlSetpointScheduleName etc. for advanced control
                    
                    logger.debug(f"Created WindowProperty:ShadingControl '{shading_ctrl.Name}'.")
                ...
                try:
                    ...
                    logger.error(f"Failed to assign Shading_Control_Name to {window_id}: {e_assign}")

            except Exception as e_fen:
                logger.error(f"Error processing blind for fenestration surface {getattr(fen, 'Name', 'UnnamedFen')}: {e_fen}")
                continue # Move to the next fenestration surface

    # --- Create Geometry-Based Shading (e.g., Overhangs, Fins) ---
    if create_geometry_shading:
        logger.info(f"Attempting to create geometry-based shading using shading_type_key: '{shading_type_key}'")
...
def _create_overhang_example(idf, building_row_context, shading_type_key, strategy, user_config_shading):
    """
    ...
    if not shading_params:
        logger.warning(f"No parameters resolved for overhang example with key '{shading_type_key}'. Skipping overhang creation.")
        return
    ...
    existing_shading_geom = [sg.Name for sg in idf.idfobjects.get("SHADING:BUILDING:DETAILED", [])]
    if overhang_name in existing_shading_geom:
        logger.info(f"Shading:Building:Detailed '{overhang_name}' already exists. Skipping creation.")
        return
    ...
    try:
        ...
        shading_obj.Vertex_1_X_coordinate = 0.0  # Assuming start at X=0 of facade segment
        shading_obj.Vertex_1_Y_coordinate = 0.0  # Y=0 is the facade plane for this local definition
        ...
        logger.debug(f"Created Shading:Building:Detailed '{shading_obj.Name}' with depth {overhang_depth}m.")

    except Exception as e:
        logger.error(f"Error creating Shading:Building:Detailed '{overhang_name}': {e}")


def add_shading_schedule(idf, schedule_name="DefaultShadingSchedule", schedule_type="Fraction"):
    """
    Example of how you might create a schedule for dynamic shading control
    (e.g., for Shading_Control_Schedule_Name in WindowProperty:ShadingControl).
    ...
    """
    logger.info(f"Attempting to create schedule: '{schedule_name}' of type '{schedule_type}'.")
    
    # Check if schedule already exists
    existing_schedules = [s.Name for s in idf.idfobjects.get("SCHEDULE:COMPACT", [])] # Also check SCHEDULE:YEAR, SCHEDULE:FILE etc.
    ...
    try:
        ...
        existing_stls = [stl.Name for stl in idf.idfobjects.get("SCHEDULETYPELIMITS", [])]
        if stl_name not in existing_stls:
            logger.warning(f"ScheduleTypeLimits '{stl_name}' not found. Creating a basic one for '{schedule_name}'.")
            ...
            # Add more types as needed
                ...
                logger.warning(f"No default setup for ScheduleTypeLimits '{schedule_type}'. It might be invalid.")
