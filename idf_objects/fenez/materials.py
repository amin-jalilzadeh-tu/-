...
def update_construction_materials(
    ...
    # These now mirror the per-element lookups and use a sub-element material
    # first before falling back to the top-level opaque material.

    def _layer_for_fallback(elem_key):
        mat = elements_data.get(elem_key, {}).get("material_opaque")
        if mat:
            return create_opaque_material(idf, mat, mat.get("Name", f"{elem_key}_Mat"))
        return opq_name
