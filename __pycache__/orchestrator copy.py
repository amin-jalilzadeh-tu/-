...
Steps:
 ...
 11. Finally, call cleanup_old_results() from cleanup_old_jobs.py to delete
     older job folders beyond a certain age.
"""
...
def orchestrate_workflow(job_config: dict, cancel_event: threading.Event = None):
    ...
    if perform_idf_creation:
        ...
        create_idfs_for_all_buildings(
            ...
            logs_base_dir=job_output_dir  # so assigned logs go to <job_output_dir>/assigned
        )
    else:
    ...
    def patch_if_relative(csv_path: str):
        ...
        if csv_path.startswith("data/"):
            # Typically your Docker container might have /usr/src/app/data
            # If you prefer another base, adjust here:
    ...
    if validation_cfg.get("perform_validation", False):
        ...
        val_conf = validation_cfg["config"]

        # If there's a sim_data_csv or real_data_csv, ensure it's absolute or data/
    ...
    if cal_cfg.get("perform_calibration", False):
        ...
        # Patch scenario_folder, real_data_csv, surrogate_model_path, etc.
    ...
    # -------------------------------------------------------------------------
    # 18) Cleanup old results in the background
    # -------------------------------------------------------------------------
    try:
        cleanup_old_results()
    except Exception as e:
        logger.error(f"[ERROR] cleanup_old_results failed => {e}")
