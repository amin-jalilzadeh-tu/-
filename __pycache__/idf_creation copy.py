...
Description:
  ...
  (plus any user/Excel overrides), and can optionally run simulations and post-process
  the results.
...
Key Steps:
  ...
  2) create_idf_for_building(...):
     ...
     - Adds fenestration (window surfaces) based on final "res_data"/"nonres_data"
     - Adds lighting, DHW, HVAC, ventilation, zone sizing, ground temps, etc.
     - Saves the resulting IDF to disk
  3) If run_simulations=True, it calls simulate_all(...) to run E+ in parallel.
  4) If post_process=True, merges the simulation outputs and writes CSV logs.

Note:
  We rely on environment variables for IDD_PATH, BASE_IDF_PATH, etc. If these are not
  provided, we fall back to default paths. You can also override them from main.py.
"""
...
###############################################################################
# Default IDF config - now referencing environment variables (with fallbacks)
###############################################################################
idf_config = {
    # Use environment variables if set, otherwise fallback to generic paths
    "iddfile":       os.getenv("IDD_PATH",       "/usr/local/EnergyPlus/Energy+.idd"),
    "idf_file_path": os.getenv("BASE_IDF_PATH",  "/app/data/Minimal.idf"),
    "output_dir":    os.path.join(os.getenv("OUTPUT_DIR", "/app/output"), "output_IDFs")
}
...
def create_idf_for_building(
    ...
    construction_map = update_construction_materials(
        ...
        strategy=strategy,
        random_seed=random_seed,
        user_config_fenez=None,  # no direct user_config_fenez needed here
...
def create_idfs_for_all_buildings(
    ...
    user_config_hvac=None,
    user_config_vent=None,
    user_config_epw=None,       # <--- if used for EPW overrides
    ...
    if run_simulations:
        ...
        base_output_dir = os.path.join(os.getenv("OUTPUT_DIR", "/app/output"), "Sim_Results")
        ...
        simulate_all(
            ...
            user_config_epw=user_config_epw,   # if used
            assigned_epw_log=assigned_epw_log,  # if needed
    ...
    if post_process:
        ...
        base_output_dir = os.path.join(os.getenv("OUTPUT_DIR", "/app/output"), "Sim_Results")
        out_csv_as_is = os.path.join(os.getenv("OUTPUT_DIR", "/app/output"), "results", "merged_as_is.csv")
        os.makedirs(os.path.dirname(out_csv_as_is), exist_ok=True)
        merge_all_results(
            base_output_dir=base_output_dir,
        ...
        # Example daily mean
        out_csv_daily_mean = os.path.join(os.getenv("OUTPUT_DIR", "/app/output"), "results", "merged_daily_mean.csv")
        os.makedirs(os.path.dirname(out_csv_daily_mean), exist_ok=True)
        merge_all_results(
            base_output_dir=base_output_dir,
...
def _write_geometry_csv(assigned_geom_log):
    ...
    out_folder = os.path.join(os.getenv("OUTPUT_DIR", "/app/output"), "assigned")
    os.makedirs(out_folder, exist_ok=True)
    out_path = os.path.join(out_folder, "assigned_geometry.csv")
...
def _write_lighting_csv(assigned_lighting_log):
    ...
    out_folder = os.path.join(os.getenv("OUTPUT_DIR", "/app/output"), "assigned")
    os.makedirs(out_folder, exist_ok=True)
    out_path = os.path.join(out_folder, "assigned_lighting.csv")
    df.to_csv(out_path, index=False)
...
def _write_fenestration_csv(assigned_fenez_log):
    ...
    out_folder = os.path.join(os.getenv("OUTPUT_DIR", "/app/output"), "assigned")
    os.makedirs(out_folder, exist_ok=True)
    out_path = os.path.join(out_folder, "assigned_fenez_params.csv")
    df.to_csv(out_path, index=False)
...
def _write_dhw_csv(assigned_dhw_log):
    ...
    out_folder = os.path.join(os.getenv("OUTPUT_DIR", "/app/output"), "assigned")
    os.makedirs(out_folder, exist_ok=True)
    out_path = os.path.join(out_folder, "assigned_dhw_params.csv")
    df.to_csv(out_path, index=False)
...
def _write_hvac_csv(assigned_hvac_log):
    ...
    out_folder = os.path.join(os.getenv("OUTPUT_DIR", "/app/output"), "assigned")
    os.makedirs(out_folder, exist_ok=True)
    out_path = os.path.join(out_folder, "assigned_hvac_params.csv")
    df.to_csv(out_path, index=False)
...
def _write_vent_csv(assigned_vent_log):
    ...
    out_folder = os.path.join(os.getenv("OUTPUT_DIR", "/app/output"), "assigned")
    os.makedirs(out_folder, exist_ok=True)
    out_path = os.path.join(out_folder, "assigned_ventilation.csv")
    df.to_csv(out_path, index=False)
